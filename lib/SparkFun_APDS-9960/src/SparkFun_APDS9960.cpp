 ...
 * relies on the Arduino SoftWire (I2C) library. to use the library, instantiate an
 ...
 */
SparkFun_APDS9960::SparkFun_APDS9960(int sda, int scl)
{
    SoftWire = new SoftI2C(sda, scl);
    gesture_ud_delta_ = 0;
    gesture_lr_delta_ = 0;
...
{
    SoftWire->beginTransmission(APDS9960_I2C_ADDR);
    SoftWire->write(val);
                                        ...
                                        unsigned int len)
{
    ...
    SoftWire->write(reg);
    for(i = 0; i < len; i++) {
        SoftWire->beginTransmission(val[i]);
    }
    if( SoftWire->endTransmission() != 0 ) {
        return false;
    }
...
bool SparkFun_APDS9960::wireReadDataByte(uint8_t reg, uint8_t &val)
{
    ...
    /* Read from register */
    SoftWire->requestFrom(APDS9960_I2C_ADDR, 1);
                                        ...
                                        unsigned int len)
{
    ...
    /* Read block data */
    SoftWire->requestFrom(APDS9960_I2C_ADDR, len);
    while (SoftWire->available()) {
        if (i >= len) {
            return -1;
        }
